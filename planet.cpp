...
Planet::Planet(const std::shared_ptr<PlanetSettings> pSetts, std::string name, int radius, int mass, sf::Vector2f position, int TOSegments, float angVelocity, int atmHeight, float atmVanishFact, sf::Color Cterrain, sf::Color Catmosphere, int amplitude, int smoothness, int peakFrequency, int peakVariation, int peakAmplitude):
    m_name(name),
    m_radius(radius),
    m_mass(mass),
    m_position(position),
